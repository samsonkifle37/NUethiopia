  ...
  try {
    ...
    const scored = places
      ...
      .map((place) => {
        ...
        if (
          interestTags.includes("culture") &&
          ["restaurant", "tour", "museum", "religious site", "landmark"].includes(place.type)
        ) {
          score += 1;
        }
    ...
    type ScoredPlace = (typeof scored)[number];

    const requestedDays = Math.min(nights || 3, 7);
    const daysArray: {
      day: number;
      morning: ScoredPlace | undefined;
      afternoon: ScoredPlace | undefined;
      evening: ScoredPlace | undefined;
    }[] = [];
    ...
    function pickFrom(
      pool: typeof scored,
      fallback: typeof scored
    ): ScoredPlace | undefined {
      let pick = pool.find((x) => !usedIds.has(x.id));

      if (!pick) {
        pick = fallback.find((x) => !usedIds.has(x.id));
      }
    ...
    const formatPlace = (r: ScoredPlace | undefined) =>
      r
        ? {
            id: r.id,
            name: r.name,
    ...
    const recommendedFlat = daysArray
